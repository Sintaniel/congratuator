...
export default function PutBox (props){
    const { register, handleSubmit } = useForm();
    const onSubmit = (data, e) => putData(data, e);
    const onError = (errors, e) => console.log(errors, e);
    let [items, setItems] = useState([]);
    const inProps =  props.launch_number;

  const putData = async(data, e) => {
      console.log(store.getState());
        await fetch(String("http://localhost:5000/api/Person").concat("", inProps),{
            method: 'PUT',
            headers: {
